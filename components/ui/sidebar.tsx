...
const SIDEBAR_MOBILE_MQ = `(max-width: 767px)`;
...
function SidebarProvider({
  ...
  const toggleSidebar = React.useCallback(() => {
    // Read live viewport so the hamburger works before useIsMobile hydrates.
    const mobile =
      typeof window !== "undefined" &&
      window.matchMedia(SIDEBAR_MOBILE_MQ).matches;
    return mobile ? setOpenMobile((v) => !v) : setOpen((v) => !v);
  }, [setOpen]);
    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn(
            "group/sidebar-wrapper flex min-h-svh w-full min-w-0 flex-1 has-data-[variant=inset]:bg-sidebar",
            className
          )}
          {...props}
        >
...
function Sidebar({
  ...
  // Until hydrated, mount both so the mobile sheet exists before useIsMobile flips.
      ...
      {showMobile ? (
        <Sheet open={openMobile} onOpenChange={setOpenMobile}>
          <SheetContent
            ...
            side={side}
            showClose={false}
            className="w-(--sidebar-width) max-w-[90vw] bg-sidebar p-0 text-sidebar-foreground pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)] [&>button]:hidden"
            style={
              {
