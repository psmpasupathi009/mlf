...
export function VoidExpenseDialog({
  ...
  async function handleSubmit() {
    ...
    setBusy(true);
    const { ok, data } = await apiFetch(`/api/expenses/${unitId}/void`, {
      method: "POST",
      json: { reason },
    });
    setBusy(false);
    if (!ok) {
      setError(
        getErrorMessage(data as Record<string, unknown>, "Failed to void entry")
      );
      return;
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent size="sm" overlayClassName="z-[60]">
        <DialogHeader>
          ...
          <DialogDescription>
            Voided expenses stay in the register for audit — they’re never
            deleted.
          </DialogDescription>
        </DialogHeader>
